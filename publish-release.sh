...
# 5.6 生成給營運看的 README.md (Release 版)
echo "📜 Generating OPS README for release branch..."
cat <<'EOF' | tr -d '\r' > deploy_temp/README.md
...
# 5.7 生成一鍵更新腳本 (update.sh)
echo "📜 Generating OPS update.sh for release branch..."
cat <<'EOF' | tr -d '\r' > deploy_temp/update.sh
#!/bin/bash
set -e
...
git add --all --force
git commit -m "Distribution: Full OPS Package with README and Update Tool at $(date '+%Y-%m-%d %H:%M:%S')"
...
echo "✨ [Antigravity] Success! Full OPS package is now live on the release branch."
echo "🔗 Operational logic: ./deploy.sh for init, ./update.sh for daily updates."
