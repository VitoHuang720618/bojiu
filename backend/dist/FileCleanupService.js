...
export class FileCleanupService {
     ...
     * @returns 是否被引用
     */
    isFileReferenced(filename, manifest) {
        if (!filename)
            return false;
     ...
     * @returns 引用的檔案路徑陣列
     */
    getReferencedPaths(manifest, assetPath) {
        const jsonPaths = this.getAllAssetPaths(manifest);
     ...
     * @returns 清理結果
     */
    async cleanupUnusedAssets(manifest) {
     ...
     */
    async overwriteOldFile(assetPath) {
        if (!assetPath)
            return null;
     ...
     * @returns 清理報告
     */
    generateCleanupReport(files, referencedPaths) {
        ...
        const details = files.map(file => {
            ...
            return {
                file,
                status: (isReferenced ? 'referenced' : 'deleted'),
