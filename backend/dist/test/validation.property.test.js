...
describe('Validation Property Tests', () => {
    ...
    describe('Property 5: File format validation', () => {
        it('should accept valid image formats', () => {
            fc.assert(fc.property(fc.constantFrom('image/png', 'image/jpg', 'image/jpeg'), fc.string({ minLength: 1, maxLength: 100 }), fc.integer({ min: 1, max: 5 * 1024 * 1024 }), // 1B to 5MB
        ...
        it('should reject invalid image formats', () => {
            fc.assert(fc.property(fc.constantFrom('text/plain', 'application/pdf', 'video/mp4', 'audio/mp3', 'image/gif', 'image/svg+xml'), fc.string({ minLength: 1, maxLength: 100 }), fc.integer({ min: 1, max: 5 * 1024 * 1024 }), (mimetype, filename, size) => {
                const mockFile = {
                    mimetype,
                    originalname: filename,
                    size,
        ...
        it('should reject files that are too large', () => {
            fc.assert(fc.property(fc.constantFrom('image/png', 'image/jpg', 'image/jpeg'), fc.string({ minLength: 1, maxLength: 100 }), fc.integer({ min: 5 * 1024 * 1024 + 1, max: 50 * 1024 * 1024 }), // > 5MB
            (mimetype, filename, size) => {
                const mockFile = {
                    mimetype,
                    originalname: filename,
        ...
        it('should throw UploadError for invalid formats in strict mode', () => {
            fc.assert(fc.property(fc.constantFrom('text/plain', 'application/pdf', 'video/mp4'), fc.string({ minLength: 1, maxLength: 100 }), fc.integer({ min: 1, max: 5 * 1024 * 1024 }), (mimetype, filename, size) => {
                const mockFile = {
                    mimetype,
                    originalname: filename,
                    size,
                ...
                try {
                    ...
                    expect(error.type).toBe(UploadErrorType.INVALID_FORMAT);
                }
            }), { numRuns: 20 });
        });
        it('should throw UploadError for oversized files in strict mode', () => {
            fc.assert(fc.property(fc.constantFrom('image/png', 'image/jpg', 'image/jpeg'), fc.string({ minLength: 1, maxLength: 100 }), fc.integer({ min: 5 * 1024 * 1024 + 1, max: 50 * 1024 * 1024 }), (mimetype, filename, size) => {
                const mockFile = {
                    mimetype,
                    originalname: filename,
                    size,
    ...
    describe('Asset path validation', () => {
        it('should accept valid asset paths', () => {
            fc.assert(fc.property(fc.string({ minLength: 1, maxLength: 50 }).filter(s => /^[a-zA-Z][a-zA-Z0-9.]*$/.test(s)), (path) => {
                expect(validateAssetPath(path)).toBe(true);
            }), { numRuns: 100 });
        });
        it('should reject invalid asset paths', () => {
            fc.assert(fc.property(fc.oneof(fc.string({ minLength: 1, maxLength: 50 }).filter(s => /^[0-9]/.test(s)), // 以數字開頭
