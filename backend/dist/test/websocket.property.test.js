...
describe('WebSocket Property Tests', () => {
    ...
    describe('Property 8: Configuration broadcast integrity', () => {
        it('should broadcast config updates to all connected clients', async () => {
            await fc.assert(fc.asyncProperty(fc.integer({ min: 1, max: 5 }), // 客戶端數量
            fc.array(fc.record({
            ...
            async (clientCount, changes) => {
                // 建立多個客戶端連接
                const clients = await Promise.all(Array(clientCount).fill(0).map(() => createClientSocket()));
                // 設置事件監聽器
                const receivedEvents = Array(clientCount).fill(0).map(() => []);
                clients.forEach((client, index) => {
                    client.on('config_updated', (event) => {
        ...
        it('should track connected clients correctly', async () => {
            await fc.assert(fc.asyncProperty(fc.integer({ min: 1, max: 10 }), async (clientCount) => {
                // 建立客戶端連接
                const clients = await Promise.all(Array(clientCount).fill(0).map(() => createClientSocket()));
                // 等待連接建立
    ...
    describe('Property 12: Concurrent editing synchronization', () => {
        it('should handle multiple simultaneous config updates', async () => {
            await fc.assert(fc.asyncProperty(fc.integer({ min: 2, max: 5 }), // 客戶端數量
            fc.array(fc.record({
                path: fc.string({ minLength: 1, maxLength: 20 }),
                oldValue: fc.webUrl(),
                newValue: fc.webUrl()
            ...
            async (clientCount, allChanges) => {
                ...
                const clients = await Promise.all(Array(clientCount).fill(0).map(() => createClientSocket()));
                // 收集所有客戶端收到的事件
                const allReceivedEvents = Array(clientCount).fill(0).map(() => []);
                clients.forEach((client, index) => {
                    client.on('config_updated', (event) => {
    ...
    describe('Room management functionality', () => {
        it('should handle room join and leave operations', async () => {
            await fc.assert(fc.asyncProperty(fc.array(fc.string({ minLength: 1, maxLength: 20 }), { minLength: 1, maxLength: 5 }), async (roomNames) => {
                const client = await createClientSocket();
                // 加入房間
        ...
        it('should broadcast to specific rooms correctly', async () => {
            ...
            const room1Events = [];
            const room2Events = [];
            client1.on('config_updated', (event) => {
                room1Events.push(event);
            });
            client2.on('config_updated', (event) => {
...
//# sourceMappingURL=websocket.property.test.js.map